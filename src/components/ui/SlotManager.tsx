...
export interface SlotManagerProps<T> {
  maxSlots: number
  currentSlot: number
  ...
  saveSettings: (slotNumber?: number) => boolean
  loadSettings?: (slotNumber: number) => T | null
  resetAll: () => void
...
interface SlotManagerState {
  slotNames: { [key: number]: string }
  tempSlotName: string
  isEditingSlotName: boolean
  slotHasData: { [key: number]: boolean }
  showUnsavedWarning: boolean
  pendingSlotNumber: number | null
}

export default function SlotManager<T>({
  maxSlots,
  currentSlot,
  slotNames: propSlotNames,
  tempSlotName: propTempSlotName,
  isEditingSlotName: propIsEditingSlotName,
  onSlotChange,
  onSlotNameChange,
  onSlotNameEdit,
  saveSettings,
  ...
  resetAll,
  hasUnsavedChanges,
  hasSlotData: propHasSlotData,
  storagePrefix
}: SlotManagerProps<T>) {
  // 내부 상태 또는 props 사용
  const [internalSlotNames, setInternalSlotNames] = useState<{ [key: number]: string }>(() => {
  ...
  const [internalTempSlotName, setInternalTempSlotName] = useState<string>('')
  const [internalIsEditingSlotName, setInternalIsEditingSlotName] = useState(false)
  ...
  // 실제 사용할 값들
  const slotNames = propSlotNames || internalSlotNames
  const tempSlotName = propTempSlotName !== undefined ? propTempSlotName : internalTempSlotName
  const isEditingSlotName = propIsEditingSlotName !== undefined ? propIsEditingSlotName : internalIsEditingSlotName

  // 슬롯 데이터 존재 여부 확인
  const hasSlotDataFunction = propHasSlotData || ((slot: number): boolean => {
    if (!storagePrefix || !canUseFunctionalCookies()) return false
    try {
  ...
  const handleSaveAndSwitch = () => {
    if (pendingSlotNumber) {
      if (saveSettings()) {
        onSlotChange(pendingSlotNumber)
      }
  ...
  const saveSlotName = () => {
    if (saveSettings()) {
      if (!propSlotNames) {
        setInternalSlotNames(prev => ({
          ...prev,
          [currentSlot]: tempSlotName
        }))
      }
      if (propIsEditingSlotName === undefined) {
        setInternalIsEditingSlotName(false)
      } else if (onSlotNameEdit) {
  ...
  const handleSlotNameChange = (name: string) => {
    if (onSlotNameChange) {
      onSlotNameChange(name)
    } else {
      setInternalTempSlotName(name)
    }
  }

  // 슬롯 이름 편집 모드 토글
  const handleSlotNameEdit = (editing: boolean) => {
    if (onSlotNameEdit) {
      onSlotNameEdit(editing)
    } else {
      setInternalIsEditingSlotName(editing)
    }
  }

  // 초기화 (내부 상태만 사용할 때)
  useEffect(() => {
    if (!propSlotNames && loadSettings && canUseFunctionalCookies()) {
      const newSlotNames: { [key: number]: string } = {}
      const newSlotHasData: { [key: number]: boolean } = {}

      for (let i = 1; i <= maxSlots; i++) {
        const settings = loadSettings(i)
        if (settings && (settings as any).slotName) {
          newSlotNames[i] = (settings as any).slotName
        } else {
          newSlotNames[i] = `슬롯 ${i}`
        }
      ...
      setInternalSlotNames(newSlotNames)
      setSlotHasData(newSlotHasData)
      setInternalTempSlotName(newSlotNames[currentSlot] || `슬롯 ${currentSlot}`)
    }
  }, [maxSlots, currentSlot, loadSettings, storagePrefix, propSlotNames])

  // 현재 슬롯이 변경될 때 tempSlotName 업데이트 (내부 상태만)
  useEffect(() => {
    if (propTempSlotName === undefined) {
      setInternalTempSlotName(slotNames[currentSlot] || `슬롯 ${currentSlot}`)
    }
  }, [currentSlot, slotNames, propTempSlotName])
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2">
              ...
              {isEditingSlotName ? (
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      ...
                      handleSlotNameEdit(false)
                      handleSlotNameChange(slotNames[currentSlot])
                    }
                  }}
                ...
                <button
                  onClick={() => handleSlotNameEdit(true)}
