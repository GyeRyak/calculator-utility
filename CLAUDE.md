...
- **한글날 훈장 행사 계산기** (`/calculators/hangeul-medal`) - 기본 슬롯 2개
  ...
  - 확률 분포 백분위 계산 (5할, 9할, 9할 9푼)
  - 평균/중앙값 재설정 횟수 및 비용 계산
  - **비용×확률 누적 방식**으로 정확한 기댓값 계산
    - 각 경로의 실제 누적 비용을 확률 가중 평균으로 추적
    - 기존 휴리스틱 추정 방식 제거, 정확도 크게 향상
  - **인터랙티브 비용 분포 차트**
    - SVG 기반 막대 그래프로 비용별 도달 확률 시각화
    - 마우스 호버 시 상세 정보 (비용, 도달 확률, 누적 확률, 상위 몇 %)
    - 한국식 확률 표기 (할/푼/리/모) 일관 적용
    - 백분위 지점 강조 표시 (5할/9할/9할 9푼 색상 구분 및 점선 가이드)
    - 누적 확률 99.9%까지만 표시 (극단값 제거)
  - **스크롤 최적화**
    - 검색어 입력 시 리스트 최상단 자동 스크롤
    - 단어 선택 시 해당 슬롯만 선택 위치로 스크롤
    - 최초 로드 시 기본 선택값으로 자동 스크롤
- **블로그 댓글 및 피드백 시스템**
